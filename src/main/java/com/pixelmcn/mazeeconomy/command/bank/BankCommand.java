...
public class BankCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command,
        ...
        switch (sub) {
            case "deposit", "dep" -> {
                if (args.length < 2) { FormatUtil.sendRaw(sender, "<gray>Usage: /bank deposit <amount|all></gray>"); return true; }
                double amount = parseAmount(args[1], plugin.getLocalEconomyManager().getWallet(player.getUniqueId()));
                if (amount <= 0) { FormatUtil.sendConfigMessage(plugin, sender, "local.bank-invalid-amount"); return true; }
                EconomyResponse resp = plugin.getLocalEconomyManager().depositToBank(player.getUniqueId(), player.getName(), amount);
                if (resp.isSuccess()) {
            ...
            case "withdraw", "with", "wd" -> {
                if (args.length < 2) { FormatUtil.sendRaw(sender, "<gray>Usage: /bank withdraw <amount|all></gray>"); return true; }
                double amount = parseAmount(args[1], plugin.getLocalEconomyManager().getBank(player.getUniqueId()));
                if (amount <= 0) { FormatUtil.sendConfigMessage(plugin, sender, "local.bank-invalid-amount"); return true; }
                EconomyResponse resp = plugin.getLocalEconomyManager().withdrawFromBank(player.getUniqueId(), player.getName(), amount);
                if (resp.isSuccess()) {
    ...
    private double parseAmount(String arg, double max) {
        if (arg.equalsIgnoreCase("all") || arg.equalsIgnoreCase("max")) return max;
        try {
    ...
    public List<String> onTabComplete(@NotNull CommandSender s, @NotNull Command c, @NotNull String l, @NotNull String[] args) {
        if (args.length == 1) return List.of("deposit", "withdraw").stream().filter(x -> x.startsWith(args[0].toLowerCase())).toList();
        if (args.length == 2) return List.of("100", "1000", "all");
        return List.of();
    }
}
